                        ...
                        <div class="flex items-center gap-4 flex-1 min-w-0">
                            <div class="w-16 h-16 bg-white p-0.5 border border-white/10 rounded-lg flex items-center justify-center overflow-hidden text-2xl shrink-0">
                                @if(isset($item->variant) && $item->variant->link_anh_bien_the)
                                    <img src="{{ $item->variant->link_anh_bien_the }}" alt="{{ $item->variant->ten_hien_thi }}" class="w-full h-full object-cover">
                                @else
                                    <i data-lucide="image" class="text-gray-400 size-6"></i>
                                @endif
                            </div>
                            <div class="min-w-0">
                                <h3 class="text-white font-bold text-base leading-snug truncate" title="{{ $item->variant->ten_hien_thi ?? 'Sản phẩm không tồn tại' }}">
                                    {{ $item->variant->ten_hien_thi ?? 'Sản phẩm không tồn tại' }}
                                </h3>
                                    ...
                                    @foreach($product->variants as $variant)
                                        <div class="variant-item flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-3 rounded-lg border border-white/5 hover:border-neon-green/30 bg-black/20 transition-colors">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p class="text-xs font-bold text-white variant-name">{{ $variant->ten_hien_thi }}</p>
                                            ...
                                            <div class="flex items-center gap-4 shrink-0 sm:justify-end">
                                                ...
                                                <button type="button" 
                                                        onclick="addVariantToFlashSale('{{ $variant->ma_bien_the }}', '{{ addslashes($variant->ten_hien_thi) }}', '{{ $variant->link_anh_bien_the }}', '{{ $variant->gia_ban }}')"
