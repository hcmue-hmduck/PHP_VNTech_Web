...
class OrderSeed extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($allUsers as $user) {
            ...
            // Shuffle để mỗi user có profile đơn hàng khác nhau
            ...
            // === Tạo review: lấy 3 đơn trong 5 đơn hoàn thành (mỗi đơn 1 review) ===
            $reviewedOrderIds = collect($completedOrderIndices)
                ->unique(fn($x) => $x['order']->ma_don_hang)
                ->shuffle()
                ->take(3)
                ->pluck('order.ma_don_hang')
                ->toArray();

            foreach ($reviewedOrderIds as $targetOrderId) {
                // Lấy ngẫu nhiên 1 item của đơn đó
                $candidates = collect($completedOrderIndices)
                    ->filter(fn($x) => $x['order']->ma_don_hang === $targetOrderId)
                    ->values();

                if ($candidates->isEmpty()) continue;
                $reviewItem = $candidates->random();
